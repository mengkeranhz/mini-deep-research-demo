 ...
 * 内部复用主模型（关流式）做一次无工具调用，产出意图/摘要/约束/信息缺口/总体计划/任务清单；
...
public class AnalyzeQueryTool implements AgentTool {
    ...
    private static final String PROMPT = """
            ...
            字段：
            - intent: 意图码「领域.动作」（如 travel.plan / research.compare / data.lookup）
            - summary: 一句话复述用户要什么
            ...
            {"intent":"travel.plan","summary":"规划国庆敦煌-德令哈 5 天行程","constraints":["国庆假期","共 5 天"],"unknowns":["出发城市","人数"],"plan":"先查门票与交通的预订规则，再排定行程","tasks":[{"content":"查莫高窟门票预订规则与放票时间","depends_on":[]},{"content":"查敦煌到德令哈的交通方式","depends_on":[]},{"content":"排定逐日行程","depends_on":[1,2]}]}
            """;
    ...
    public ToolDef definition() {
        return new ToolDef(name(), "分析用户述求：识别意图、硬约束与信息缺口，给出总体计划并拆解为带依赖的任务清单（JSON）。"
                        + "复杂任务在开始执行前调用一次；生成的任务状态每轮以系统快照自动注入。",
                Map.of("type", "object",
                        "properties", Map.of(
                                "query", Map.of("type", "string", "description", "用户述求原文")),
    ...
    public String execute(JsonNode input) throws Exception {
        ...
        String raw = LlmClient.create(quiet).call(List.of(),
                List.of(Msg.system(PROMPT), Msg.user(query))).text();
        ...
        tasks.reset(new TaskStore.Header(root.path("intent").asText(""), root.path("plan").asText(""),
                strList(root.path("unknowns"))), parsed);

        // 归一化后的 JSON 回给模型（含分配的 id 与初始状态）
        ObjectNode out = M.createObjectNode();
        out.put("intent", root.path("intent").asText(""));
