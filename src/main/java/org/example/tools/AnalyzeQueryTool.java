...
public class AnalyzeQueryTool implements AgentTool {
    ...
    private static final String PROMPT = """
            ...
            - tasks: 3-6 个可执行任务，每项为一个任务描述字符串。
              ...
              mode=orchestrator 时每个任务由独立子代理执行——子代理只能看到该任务文本与事实账本（看不到原始述求），任务描述必须自包含：写明背景、范围、时期、口径、单位与期望产出；
              mode=single 时任务由父代理亲自执行。
            述求中的事实性断言（某数据存在、按某频率公布、某口径可得）是待核实假设而非事实：规划时先安排低成本核查（证实或证伪通常只需一两个权威来源），存在性未核实的不要直接规划成「从中取数」的取数任务。
            若提供了当前进度或事实账本：结合已知信息规划，只补剩余工作；账本已有否定性结论（某数据不存在、无法查到、命题被证伪、公布频率或口径与述求所设不符）的不再重复安排相应检索，据其调整目标并注明所依据的账本事实；目标需要调整时按调整后的目标给出。
            """;
    ...
    public String execute(JsonNode input) throws Exception {
        ...
        Config.Llm quiet = new Config.Llm(llmCfg.provider(), llmCfg.baseUrl(), llmCfg.model(),
                llmCfg.apiKey(), llmCfg.maxTokens(), llmCfg.temperature(), false, 0);
