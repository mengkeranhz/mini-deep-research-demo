...
final class FinalVerifier {
    ...
    private static final String PROMPT = """
            ...
            2. 不可得述求的裁决：述求要求数据而账本有带来源的否定性结论（该数据不存在、经充分检索仍无法查到、述求所设命题被证伪、公布频率或口径与述求所设不符）时，草稿如实报告该结论并给出可得口径或替代方案，即算回答到位；草稿声称不可得而账本无对应证据的，算缺陷。账本中另有与该否定性结论冲突的旁证（如其他来源已引用或使用该数据的具体数值）时，草稿仍直接断言该数据「不存在／未发布」的，属证据冲突，算缺陷——须降级为「未能定位一手来源」并如实呈现旁证数值与来源，或先复核再下结论；本条仅裁决数据可得性类结论（不存在、无法查到、公布频率或口径不符），命题证伪类结论不适用。
